...
//! 1. The `sd_notify` API which can ask rustysd to store some file descriptors so they stay open over restarts
use std::{
    collections::HashMap,
...
pub struct FDStore {
    ...
    service_stored_sockets: HashMap<String, HashMap<String, Vec<RawFd>>>,
}
...
impl FDStore {
    ...
    pub fn insert_global(&mut self, name: String, new_fds: GlobalEntry) -> Option<GlobalEntry> {
        if let std::collections::hash_map::Entry::Vacant(e) = self.global_sockets.entry(name) {
            e.insert(new_fds);
            None
        } else {
            Some(new_fds)
    ...
    /// normal get semantics on a hashmap
    #[must_use]
    pub fn get_service_stored(&self, srvc_name: &String, fd_name: &String) -> Option<&Vec<RawFd>> {
