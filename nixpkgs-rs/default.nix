{
  ...
  overlays.nixpkgs-rs = final: prev: let
  ...
  in {
    ...
    nixpkgs-rs-components = components;
  ...
  packages = {
    ...
    nixpkgs-rs-test = {
      ...
      stdenv.mkDerivation {
        ...
        buildPhase = ''
          echo "=== nixpkgs-rs component status ==="
          echo ""
        ...
        installPhase = ''
          mkdir -p $out
          echo "nixpkgs-rs component test passed" > $out/result
        '';

        meta = {
          description = "Test derivation for nixpkgs-rs component availability";
