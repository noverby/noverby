...
`nixpkgs-rs` is the build-time counterpart to [nixos-rs](../nixos-rs). Where nixos-rs replaces runtime system components (systemd, bash, coreutils) in a running NixOS system, nixpkgs-rs replaces the **build tools** — the stdenv toolchain that Nix uses to compile and package software.
...
2. Add a `default.nix` following the pattern in [pkg-config-rs](../pkg-config-rs) or [systemd-rs](../systemd-rs)
...
5. Add `nixpkgs-rs` scope to your commit: `feat(nixpkgs-rs): Wire in make-rs replacement`
...
| [nixos-rs](../nixos-rs) | Runtime system (NixOS) | Replace components in the running OS |
| [systemd-rs](../systemd-rs) | Init system | Full systemd rewrite, used by nixos-rs |
| [pkg-config-rs](../pkg-config-rs) | Build dependency lookup | Drop-in pkg-config replacement |
