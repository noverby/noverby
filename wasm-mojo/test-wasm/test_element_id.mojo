# ElementIdAllocator exercised through the real WASM binary via wasmtime-py
...
#
# Run with:
#   mojo test test-wasm/test_element_id.mojo
...
fn _create_alloc(w: PythonObject) raises -> PythonObject:
    """Create a heap-allocated ElementIdAllocator via WASM."""
    return w.eid_alloc_create()


fn _destroy_alloc(w: PythonObject, alloc_ptr: PythonObject) raises:
    """Destroy a heap-allocated ElementIdAllocator via WASM."""
    w.eid_alloc_destroy(alloc_ptr)
...
fn test_allocator_initial_state() raises:
    ...
    assert_equal(Int(w.eid_count(a)), 1, "initial count should be 1 (root)")
    assert_equal(Int(w.eid_user_count(a)), 0, "initial user_count should be 0")
...
fn test_allocator_first_alloc_returns_1() raises:
    ...
    var id = Int(w.eid_alloc(a))
    assert_equal(id, 1, "first alloc should return ID 1")
    assert_equal(Int(w.eid_count(a)), 2)
    assert_equal(Int(w.eid_user_count(a)), 1)
...
fn test_allocator_sequential_alloc() raises:
    ...
    var id1 = Int(w.eid_alloc(a))
    var id2 = Int(w.eid_alloc(a))
    var id3 = Int(w.eid_alloc(a))

    assert_equal(id1, 1)
    assert_equal(id2, 2)
    assert_equal(id3, 3)
    assert_equal(Int(w.eid_count(a)), 4)
    assert_equal(Int(w.eid_user_count(a)), 3)
...
fn test_allocator_is_alive() raises:
    ...
    assert_equal(Int(w.eid_is_alive(a, id1)), 1, "id1 should be alive")
    assert_equal(Int(w.eid_is_alive(a, id2)), 1, "id2 should be alive")
    assert_equal(Int(w.eid_is_alive(a, 0)), 1, "root should be alive")
    assert_equal(
        Int(w.eid_is_alive(a, 99)), 0, "unallocated ID should not be alive"
    )
...
fn test_allocator_free_decrements_count() raises:
    ...
    var id1 = Int(w.eid_alloc(a))
    var id2 = Int(w.eid_alloc(a))
    assert_equal(Int(w.eid_user_count(a)), 2)

    w.eid_free(a, id1)
    assert_equal(Int(w.eid_user_count(a)), 1)
    assert_equal(Int(w.eid_is_alive(a, id1)), 0, "freed ID should not be alive")
    assert_equal(
        Int(w.eid_is_alive(a, id2)), 1, "other ID should still be alive"
    )
...
fn test_allocator_free_root_is_noop() raises:
    ...
    _ = w.eid_alloc(a)
    var count_before = Int(w.eid_count(a))
...
fn test_allocator_double_free_is_noop() raises:
    ...
    var id = Int(w.eid_alloc(a))
    w.eid_free(a, id)
    var count_after_first_free = Int(w.eid_count(a))

    w.eid_free(a, id)  # double free
    assert_equal(
        Int(w.eid_count(a)),
...
fn test_allocator_reuse_freed_slot() raises:
    ...
    var id1 = Int(w.eid_alloc(a))  # gets 1
    var id2 = Int(w.eid_alloc(a))  # gets 2

    w.eid_free(a, id1)  # free slot 1

    var id3 = Int(w.eid_alloc(a))  # should reuse slot 1
    assert_equal(id3, id1, "new alloc should reuse freed slot")
    assert_equal(Int(w.eid_is_alive(a, id3)), 1, "reused ID should be alive")
    assert_equal(
        Int(w.eid_is_alive(a, id2)), 1, "other ID should still be alive"
    )
...
fn test_allocator_reuse_multiple_freed_slots() raises:
    ...
    var id1 = Int(w.eid_alloc(a))  # 1
    _ = w.eid_alloc(a)  # 2
    var id3 = Int(w.eid_alloc(a))  # 3

    w.eid_free(a, id1)
    w.eid_free(a, id3)
    assert_equal(Int(w.eid_user_count(a)), 1, "only id2 should remain")

    # Allocate two more — should reuse freed slots (LIFO order from free list)
    var id4 = Int(w.eid_alloc(a))
    var id5 = Int(w.eid_alloc(a))
    assert_equal(Int(w.eid_is_alive(a, id4)), 1)
    assert_equal(Int(w.eid_is_alive(a, id5)), 1)
    assert_equal(Int(w.eid_user_count(a)), 3)
...
fn test_allocator_stress_100() raises:
    ...
    var ids = List[Int]()
    for i in range(100):
        ids.append(Int(w.eid_alloc(a)))

    assert_equal(Int(w.eid_user_count(a)), 100)

    # All IDs should be unique and alive
    for i in range(100):
        assert_equal(
            Int(w.eid_is_alive(a, ids[i])), 1, "id " + String(i) + " is alive"
        )
...
fn test_allocator_stress_free_even_realloc() raises:
    ...
    assert_equal(
        Int(w.eid_user_count(a)), 25, "25 should remain after freeing 25"
    )

    # Allocate 25 more — should reuse freed slots
    var new_ids = List[Int]()
    for i in range(25):
        new_ids.append(Int(w.eid_alloc(a)))

    assert_equal(Int(w.eid_user_count(a)), 50, "back to 50 after reallocating")

    # All new IDs should be alive
    for i in range(25):
        assert_equal(
            Int(w.eid_is_alive(a, new_ids[i])),
            1,
...
fn test_allocator_stress_alloc_free_cycle() raises:
    ...
    for _ in range(1000):
        var id = Int(w.eid_alloc(a))
        assert_equal(Int(w.eid_is_alive(a, id)), 1)
        w.eid_free(a, id)

    # After all cycles, should be back to just root
    assert_equal(Int(w.eid_user_count(a)), 0)
    # But one slot was created and is on the free list
    assert_equal(Int(w.eid_count(a)), 1)
