...
> **See [README.md § Architecture](README.md#architecture)** for the current architecture
> diagram and responsibility split. The README is the source of truth for what was
> actually built; this file documents what was *planned* at each phase.
...
> **See [README.md § Test infrastructure](README.md#test-infrastructure)** and
> **[README.md § Test results](README.md#test-results)** for the current test
> setup, module list, and pass counts. The sections below were the original
> planned structure; the actual layout is flat (not nested by phase).
...
> current file tree. The sections below were the original planned structure;
> the actual layout diverged during implementation.
...
- [x] All Mojo protocol tests updated for new wire format size
- [x] All JS protocol tests updated for new field
- [x] All mutation tests updated for new `new_event_listener` signature
- [x] Verify handler_id flows through counter create → JS decode
...
- [x] `AppShell.emit_templates(writer_ptr)` method
- [x] `Runtime.template_count()` / `Runtime.get_template(index)` accessors (already existed: `TemplateRegistry.count()` + `get_ptr(id)`)
- [x] Integration into `*_rebuild()` (prepend strategy) — all three apps emit templates before CreateEngine
- [x] Tests: verify templates appear before `LoadTemplate` in mutation buffer (counter + todo)
...
### 13.5 AppShell Memo Helpers (✅ Done)
...
### 13.6 Counter App Memo Demo (✅ Done)
